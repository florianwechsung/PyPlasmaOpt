...
void biot_savart_by_dcoilcoeff_all_simd(vector_type& pointsx, vector_type& pointsy, vector_type& pointsz, T& gamma, T& dgamma_by_dphi, T& dgamma_by_dcoeff, T& d2gamma_by_dphidcoeff, T& dB_by_dcoilcoeff, T& d2B_by_dXdcoilcoeff) {
    ...
    int num_coil_coeffs = dgamma_by_dcoeff.shape(2);
    ...
    for(int i = 0; i < num_points-num_points%simd_size; i += simd_size) {
        ...
        for(int k=0; k<num_coil_coeffs; k++) {
            ...
            for (int j = 0; j < num_quad_points; ++j) {
                ...
                auto d2gamma_j_by_dphi_dcoeff_k = Vec3d{ d2gamma_by_dphidcoeff(j, 0, k), d2gamma_by_dphidcoeff(j, 1, k), d2gamma_by_dphidcoeff(j, 2, k) };
                auto dgamma_j_by_dcoeff_k = Vec3d{ dgamma_by_dcoeff(j, 0, k), dgamma_by_dcoeff(j, 1, k), dgamma_by_dcoeff(j, 2, k) };
            ...
            for(int j=0; j<simd_size; j++){
                dB_by_dcoilcoeff(i+j, 0, k) = B_i.x[j];
                dB_by_dcoilcoeff(i+j, 1, k) = B_i.y[j];
                dB_by_dcoilcoeff(i+j, 2, k) = B_i.z[j];
                for (int l = 0; l < 3; ++l) {
                    d2B_by_dXdcoilcoeff(i+j, l, 0, k) += dB_dX_i[l].x[j];
                    d2B_by_dXdcoilcoeff(i+j, l, 1, k) += dB_dX_i[l].y[j];
                    d2B_by_dXdcoilcoeff(i+j, l, 2, k) += dB_dX_i[l].z[j];
                }
            }
        }
    }
    for (int i = num_points - num_points % simd_size; i < num_points; ++i) {
        ...
        for (int j = 0; j < num_quad_points; ++j) {
            ...
            for(int k=0; k<num_coil_coeffs; k++) {
                ...
                auto temp = term1 - term2 + term3;
                dB_by_dcoilcoeff(i, 0, k) += temp[0];
                dB_by_dcoilcoeff(i, 1, k) += temp[1];
                dB_by_dcoilcoeff(i, 2, k) += temp[2];
                for (int l = 0; l < 3; ++l) {
                    auto el  = Vec3d{0., 0., 0.};
                    el[l] = 1.0;
                    ...
                    auto temp = term1 + term2 + term3 + term4 + term5 + term6;
                    d2B_by_dXdcoilcoeff(i, l, 0, k) += temp[0];
                    d2B_by_dXdcoilcoeff(i, l, 1, k) += temp[1];
                    d2B_by_dXdcoilcoeff(i, l, 2, k) += temp[2];
                }
            }
        }
    }
...
//                    for(int k=0; k<simd_size; k++){
//                        res_grad_gamma(j, d) += temp1vi[k];
//                        res_grad_dgamma_by_dphi(j, d) += temp2vi[k];
//                    }
//                }
//            }
//        }
...
//                    auto temp1vi = inner(temp1, vgrad_i[l]);
//                    auto temp2vi = inner(temp2, vgrad_i[l]);
//                    res_grad_gamma(j, d) += temp1vi;
//                    res_grad_dgamma_by_dphi(j, d) += temp2vi;
//                }
//            }
//        }
//    }
    ...
    for(int i = 0; i < num_points-num_points%simd_size; i += simd_size) {
        ...
        for (int j = 0; j < num_quad_points; ++j) {
            ...
            for(int d=0; d<3; d++){
                ...
                auto term1 = cross(d, diff) * norm_diff_3_inv; // cross(d2gamma_j_by_dphi_dcoeff_k, diff) * norm_diff_3_inv;
                ...
                auto diff_inner_dgamma_j_by_dcoeff_k = inner(d, diff);//inner(dgamma_j_by_dcoeff_k, diff);
                auto term3 = three_dgamma_by_dphi_cross_diff * diff_inner_dgamma_j_by_dcoeff_k * norm_diff_5_inv ;

                auto res_dgamma_by_dphi_add = inner(term1, v_i);
                auto res_gamma_add = inner(term3, v_i)-inner(term2, v_i);

                res_gamma(j, d) += xsimd::hadd(res_gamma_add); // = \sum_k res_gamma_add[k]
                res_dgamma_by_dphi(j, d) += xsimd::hadd(res_dgamma_by_dphi_add);
                //for(int k=0; k<simd_size; k++){
                    //res_gamma(j, d) += res_gamma_add[k];
                    //res_dgamma_by_dphi(j, d) += res_dgamma_by_dphi_add[k];
                //}
                ...
                for (int l = 0; l < 3; ++l) {
                    ...
                    auto d2gamma_j_by_dphi_dcoeff_k_cross_el = cross(ed, el);//cross(d2gamma_j_by_dphi_dcoeff_k, el);
                    auto dgamma_j_by_dphi_cross_el = cross(dgamma_j_by_dphi, el);
                    ...
                    auto temp1 = Vec3dSimd(dgamma_j_by_dphi_cross_el[0], dgamma_j_by_dphi_cross_el[1], dgamma_j_by_dphi_cross_el[2]) * (3. * diff_inner_dgamma_j_by_dcoeff_k * norm_diff_5_inv);
                    temp1 += three_dgamma_by_dphi_cross_diff * (-5. * diff_inner_dgamma_j_by_dcoeff_k * diff[l] * norm_diff_7_inv);
                    temp1 += three_dgamma_by_dphi_cross_diff * (dgamma_j_by_dcoeff_k[l] * norm_diff_5_inv);
                    auto term6_fak = 3. * diff[l] * norm_diff_5_inv;
                    temp1 += Vec3dSimd(dgamma_j_by_dphi_cross_dgamma_j_by_dcoeff_k[0] * term6_fak,
                            dgamma_j_by_dphi_cross_dgamma_j_by_dcoeff_k[1] * term6_fak,
                            dgamma_j_by_dphi_cross_dgamma_j_by_dcoeff_k[2] * term6_fak);

                    auto temp2 = Vec3dSimd(d2gamma_j_by_dphi_dcoeff_k_cross_el[0], d2gamma_j_by_dphi_dcoeff_k_cross_el[1], d2gamma_j_by_dphi_dcoeff_k_cross_el[2]) * norm_diff_3_inv;
                    temp2 += d2gamma_j_by_dphi_dcoeff_k_cross_diff * (-3. * diff[l] * norm_diff_5_inv);
                    
                    res_grad_gamma(j, d) += xsimd::hadd(inner(temp1, vgrad_i[l]));
                    res_grad_dgamma_by_dphi(j, d) += xsimd::hadd(inner(temp2, vgrad_i[l]));
                    //auto temp1vi = inner(temp1, vgrad_i[l]);
                    //auto temp2vi = inner(temp2, vgrad_i[l]);
                    //for(int k=0; k<simd_size; k++){
