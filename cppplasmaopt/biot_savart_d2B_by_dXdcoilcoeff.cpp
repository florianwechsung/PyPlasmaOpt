        ...
        for (int j = 0; j < num_quad_points; ++j) {
            ...
            for(int k=0; k<num_coil_coeffs; k++) {
                ...
                for (int l = 0; l < 3; ++l) {
                    ...
                    auto term2 = 3 * inner(diff, dgamma_j_by_dcoeff_k) * norm_diff_5_inv * cross(dgamma_j_by_dphi, el);
                    auto term3 = -15 * inner(diff, dgamma_j_by_dcoeff_k) * diff.at(l) * norm_diff_7_inv * dgamma_by_dphi_cross_diff;
                    auto term4 = 3 * dgamma_j_by_dcoeff_k.at(l) * norm_diff_5_inv * dgamma_by_dphi_cross_diff;
                    auto term5 = -3 * diff.at(l) * norm_diff_5_inv * cross(d2gamma_j_by_dphi_dcoeff_k, diff);
                    auto term6 = 3 * diff.at(l) * norm_diff_5_inv * cross(dgamma_j_by_dphi, dgamma_j_by_dcoeff_k);
