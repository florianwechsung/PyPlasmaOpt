...
out_of_sample_values = np.loadtxt(outdir + "out_of_sample_values.txt")
xiterates = np.loadtxt(outdir + "xiterates.txt")
niterates = len(Jvals)
Jvals_outofsample = np.loadtxt(outdir + "Jvals_outofsample.txt")
...
# plt.semilogy(list(range(0, len(Jvals), 25)), np.mean(out_of_sample_values, axis=1), ":", label="OoS Mean")
print(np.mean(out_of_sample_values, axis=1))
print("%e" % np.mean(Jvals_outofsample))
plt.legend()
plt.grid()
ymin, ymax = plt.ylim()
logmin = floor(log10(1.1e-9))
# logmin = floor(log10(ymin))
logmax = ceil(log10(ymax))
ax = plt.gca()
ax.set_yticks([10**(i) for i in range(logmin, logmax+1)])
plt.title("Convergence " + args.mode)
plt.tight_layout()
