...
def get_24_coil_data(Nt_coils=5, Nt_ma=3, nfp=2, ppp=10, at_optimum=False):
    ...
    coils = [FourierCurve((Nt_coils+1)*ppp, Nt_coils) for i in range(num_coils)]
...
def get_ncsx_data(Nt=25, ppp=10, case='orig'):
    assert case in ['orig', 'optim_no_reg']
    dir_path = os.path.dirname(os.path.realpath(__file__))

    if case == 'orig':
        coil_data = np.loadtxt(os.path.join(dir_path, "data", "NCSX_coil_coeffs.dat"), delimiter=',')
    elif case == 'optim_no_reg':
        coil_data = np.loadtxt(os.path.join(dir_path, "data", "NCSX_coil_coeffs_optim_no_reg.dat"), delimiter=' ')
    nfp = 3
    num_coils = 3
    coils = [FourierCurve(Nt*ppp, Nt) for i in range(num_coils)]
    for ic in range(num_coils):
        dofs = coils[ic].dofs
        dofs[0][0] = coil_data[0, 6*ic + 1]
        dofs[1][0] = coil_data[0, 6*ic + 3]
    ...
    numpoints = Nt*ppp+1 if ((Nt*ppp) % 2 == 0) else Nt*ppp
    ma = StelleratorSymmetricCylindricalFourierCurve(numpoints, Nt, nfp)
    dofs = ma.dofs
    if case == 'orig':
...
def get_16_coil_data(Nt=10, ppp=10, at_optimum=False):
    ...
    nfp = 2
    num_coils = 4
    coils = [CartesianFourierCurve(Nt*ppp, Nt) for i in range(num_coils)]
    for ic in range(num_coils):
        dofs = coils[ic].dofs
        dofs[0][0] = coil_data[0, 6*ic + 1]
        dofs[1][0] = coil_data[0, 6*ic + 3]
