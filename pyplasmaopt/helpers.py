...
def get_flat_data(Nt=4, nfp=3, ppp=10):
    ...
    num_coils = 3
    coils = [CartesianCurveXYZFourier(Nt, np.linspace(0, 1, Nt*ppp, endpoint=False)) for i in range(num_coils)]
    for ic in range(num_coils):
    ...
    ma = CurveRZFourier(Nt, nfp, np.linspace(0, 1/nfp, numpoints, endpoint=False), True)
...
def get_16_coil_data(Nt=10, ppp=10, at_optimum=False):
    ...
    nfp = 2
    num_coils = 4
    coils = [CartesianCurveXYZFourier(Nt*ppp, Nt) for i in range(num_coils)]
    for ic in range(num_coils):
        dofs = coils[ic].dofs
        dofs[0][0] = coil_data[0, 6*ic + 1]
        dofs[1][0] = coil_data[0, 6*ic + 3]
    ...
    numpoints = Nt*ppp+1 if ((Nt*ppp) % 2 == 0) else Nt*ppp
    ma = CurveRZFourier(numpoints, Nt, nfp, True)
    if at_optimum:
