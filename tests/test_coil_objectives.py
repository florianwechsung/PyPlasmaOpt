import pytest
import numpy as np
from pyplasmaopt import CartesianFourierCurve, CurveLength, CurveCurvature, CurveTorsion
...
def get_coil():
    ...
    dofs = coil.get_dofs()
    coil.set_dofs(dofs + 0.01 * np.random.rand(len(dofs)).reshape(dofs.shape))
    return coil
