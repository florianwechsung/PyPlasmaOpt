...
def get_coil():
    ...
    cfc = CartesianFourierCurve(3, np.asarray([0.5]))
...
def test_coil_first_derivative():
    cfc = get_coil()
    x = np.asarray([1])
    taylor_test(lambda p: cfc.gamma_impl(p), lambda p: cfc.dgamma_by_dphi_impl(p), x)
...
def test_coil_coefficient_derivative():
    ...
    def df(dofs):
        cfc.set_dofs(dofs)
        return cfc.dgamma_by_dcoeff
    taylor_test(f, df, coeffs)

    def f(dofs):
        cfc.set_dofs(dofs)
        return cfc.dgamma_by_dphi[:, 0, :]
    def df(dofs):
        cfc.set_dofs(dofs)
        return cfc.d2gamma_by_dphidcoeff[:, 0, :, :]
    taylor_test(f, df, coeffs)
...
def test_coil_curvature_derivative():
    ...
    def df(dofs):
        cfc.set_dofs(dofs)
        return cfc.dkappa_by_dcoeff
    taylor_test(f, df, coeffs)

def get_magnetic_axis():
    from pyplasmaopt import StelleratorSymmetricCylindricalFourierCurve

    ma = StelleratorSymmetricCylindricalFourierCurve(3, 2, np.asarray([1]))
...
def test_magnetic_axis_first_derivative():
    ...
    x = np.asarray([1])
    taylor_test(lambda p: ma.gamma_impl(p), lambda p: ma.dgamma_by_dphi_impl(p), x)
...
def test_magnetic_axis_incremental_arclength_derivative():
    ...
    def f(dofs):
        ma.set_dofs(dofs)
        return ma.incremental_arclength
    def df(dofs):
        ma.set_dofs(dofs)
        return ma.dincremental_arclength_by_dcoeff
    taylor_test(f, df, coeffs)
...
if __name__ == "__main__":
    ...
    x = ma.gamma_impl(points)
    (t, n, b) = ma.frenet_frame_impl(points)
